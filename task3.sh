# Usage: bash task3.sh
college=$(cat wages.csv | grep -E "([a-z]+\,[0-9]+\,16\,[0-9]+)" | sort -n -t , -k 4 | head -n 1 | cut -d , -f 4)
...
highschool=$(cat wages.csv | grep -E "([a-z]+\,[0-9]+\,12\,[0-9]+)" | sort -n -t , -k 4 | head -n 1 | cut -d , -f 4)
